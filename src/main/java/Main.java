...
public class Main {
    public static void main(String[] args) {
        ...
        //Arrays.asList(paragraphs).forEach(el -> System.out.println(el + "\n-"));
        String oneP = paragraphs[0];

        String[] sentences = oneP.split("(?<=(?<![A-Z])\\. )"); // параграф разбить по .?! на предложения
        // Arrays.asList(sentences).forEach(el -> System.out.println(el + "\n-"));

        String oneSent = sentences[0].trim().replaceAll("\\s+", " "); // убрать лишние пробелы

        String[] tokens = oneSent.split(" ");
        Arrays.asList(tokens).forEach(el -> System.out.println(el + "\n-"));
